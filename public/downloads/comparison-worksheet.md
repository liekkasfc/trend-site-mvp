...
- Solo use: One buyer can cut through vendor sprawl quickly by logging first choice, fallback, and reject reasons in one place.
- Team use: Use the weighted score and decision log so reviewers can disagree on one sheet instead of across scattered tabs.
- Client work: Use the commercial notes to document hidden cost, upgrade trigger, and unresolved risk before presenting a recommendation externally.

## Blank template preview
- Contender columns: Blank rows for three to four real options plus one production-shaped use case.
- Weighted score fields: Blank scores for time-to-value, workflow friction, pricing clarity, review drag, and reuse potential.
- Decision log shell: Blank fields for first choice, fallback, reject reasons, hidden cost, and upgrade trigger.
