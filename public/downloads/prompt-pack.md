...
- Variable prompt matrix: Prompt blocks for hooks, screenshot sequence, transitions, CTA framing, and variable placeholders that map directly to the first publish-ready short-form demo pass.
- Repair prompts: Fallback prompts for generic output, weak motion, unclear CTA framing, or sequence drift after the first pass.
- Review rubric: A compact QA rubric for clarity, motion quality, sequencing, and CTA placement before the clip leaves review.
...
- Solo use: One operator can copy the intake block, run one launch clip, and save the winning hook plus review delta for the second pass.
- Team use: Use the review rubric and reuse notes so product marketing, creative, and approvers judge the same structure instead of reinventing the bar in chat.
- Client work: Use the intake block and reviewer note to freeze the deliverable definition early, then save the revision pattern so the next client brief starts cleaner.

## Blank template preview
- Blank intake fields: Source asset, target channel, conversion goal, reviewer, and publish-ready definition.
- Prompt variable slots: Hook angle, sequence beats, CTA frame, proof cue, and repair prompt slots left blank for the first live use case.
- Review note stub: A fill-in area for what changed after review, what still failed, and what the next teammate should preserve.
